...
public class CashItemAggregator
{
    ...
    public void aggregateBudget(XSSFWorkbook budgetWorkbook, HashMap<String, Integer> pandLmap, int targetMonth)
    {
        ...
        for (int i = 0; i < budgetSheet.getLastRowNum() - 1; i++)
        {
            ...
            {
                System.out.println("Exception at 109...end reading budget rows at row " + i);
            }
            switch (switchKey)
            {
                case "Total 43400 Direct Public Support":
                    currentBudgetCell.setCellType(CELL_TYPE_NUMERIC);//numeric
                ...
                case "Paying Students (Budget)":
                   try
                   {
                       ...
                       System.out.println("Paying Students (Budget) cell type for column " + currentBudgetCell.getColumnIndex() + " is "+ currentBudgetCell.getCellType() + " value is " + currentBudgetCell.getNumericCellValue());
                       payingStudentsVariance = payingStudentsActual - (int) payingStudentsBudget;
                       System.out.printf("%-40s %-20d %n", "Paying Students (Budget)", payingStudentsBudget);
                       monthVarianceCell.setCellValue(payingStudentsVariance);
                   }
                   catch (Exception e)
                   {
                       System.out.println("Paying Students (Budget) Exception => " + e);
                   }
                    break;
                case "Paying Students (Actual)":
                    ...
                    {
                        System.out.println("Exception in Paying Students (Actual)...probably empty budget cell => " + e);
                    }
                    break;
                default:
            }
